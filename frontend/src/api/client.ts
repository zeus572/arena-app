...
// TOTP two-factor authentication. Enrollment endpoints require an authenticated user;
// the login challenge itself lives in AuthContext (it runs pre-session).
...
export async function mfaEnable(code: string) {
  const res = await api.post<{ status: string; backupCodes: string[] }>("/auth/mfa/enable", { code });
  return res.data;
}

export async function mfaDisable(password: string) {
  const res = await api.post<{ status: string }>("/auth/mfa/disable", { password });
  return res.data;
}

export async function regenerateBackupCodes(password: string) {
  const res = await api.post<{ backupCodes: string[] }>("/auth/mfa/backup-codes", { password });
  return res.data;
}
